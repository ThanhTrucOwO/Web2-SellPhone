    ...
    <div class="main">
        <div class="timkiem">
            ...
            <form name="timkiem" method="get" action="product-find.php">
                <div>
                    ...
                    <input type="text" name="keyword_advanced" value="<?php echo $keyword_advanced;?>">
                </div>
                <div>
                    <p class="type_find">Khoảng giá từ</p>
                    <input type="number" value="<?php echo $price_start;?>" name="price_start">
                    <p class="type_find">Đến</p>
                    <input type="number" value="<?php echo $price_end;?>" name="price_end">
                </div>
                <div>
                    <p class="type_find">Danh mục</p>
                    <?php while($cate = mysqli_fetch_assoc($category_active_list)): ?>
                    <div>
                        <input type="radio" id="<?php echo $cate['category_id']; ?>" name="drone" value="<?php echo $cate['category_id']; ?>" <?php echo $cate['category_id']==$option?"checked":""; ?>>
        ...
        <div class="product">
            <div class="product_hot">
                ...
                <?php for($i = 0; $i < $limit && ($start + $i) < $total_record ; $i = $i + 1): ?> 
                    <?php $product = $product_list[$start + $i]; ?>
            ...
            <div class="phantrang">
                ...
                <?php if($current_page > 1 && $total_page > 1): ?>
                    <li><a href="product-find.php?keyword_advanced=<?php echo $keyword_advanced; ?>&price_start=<?php echo $price_start;?>&price_end=<?php echo $price_end; ?>&drone=<?php echo $option; ?>&btnTimKiem=Tìm+kiếm&pagefind=<?php echo $current_page - 1; ?>">Prev</a></li>
                <?php endif; ?>    
                <?php $i = 1; while($i<=$total_page): ?>
                    <li><a class="<?php echo $i==$current_page?"select":""; ?>" href="product-find.php?keyword_advanced=<?php echo $keyword_advanced; ?>&price_start=<?php echo $price_start;?>&price_end=<?php echo $price_end; ?>&drone=<?php echo $option; ?>&btnTimKiem=Tìm+kiếm&pagefind=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                <?php $i = $i + 1; endwhile; ?>
                <?php if($current_page < $total_page && $total_page > 1): ?>
                    <li><a href="product-find.php?keyword_advanced=<?php echo $keyword_advanced; ?>&price_start=<?php echo $price_start;?>&price_end=<?php echo $price_end; ?>&drone=<?php echo $option; ?>&btnTimKiem=Tìm+kiếm&pagefind=<?php echo $current_page + 1; ?>">Next</a></li>
                <?php endif; ?>
                </ul>            
            </div>        
        </div>
